...
function build_protocol_list( $disease_site_desc ) {
    global $PROTOCOL_LIST_URL,$MAINTAINANCE_MSG;
    $url               = esc_url_raw( $PROTOCOL_LIST_URL . $disease_site_desc );
...
function protocol_list_short_code( $args ) {
    global $WARNING_MESSAGE,$PROTOCOL_LISTING_WM;
    if ( isset( $args['disease_site_desc'] ) ) {
        ...
        if ( is_null( $pl_request ) ) {
            return '<h4>'.$WARNING_MESSAGE.'</h4>';
        }
        return $pl_request;
    } else {
        return '<h4>'.$PROTOCOL_LISTING_WM.'</h4>';
    }
}
...
function build_protocol_summary( $protocol, $clinical_trial ) {
    global $PROTOCOL_LABEL,$PHASE_LABEL,$SPONSOR_LABEL,$PROTOCOL_TITLE_LABEL,$PI_LABEL,$SCOPE_LABEL,$DESCRIPTION_LABEL,$GENDER_LABEL,
           $AGE_LABEL,$CONDITIONS_LABEL,$CTGOV_LABEL,$INST_LABEL,$MORE_INFO_LABEL,$OBJECTIVE_LABEL,$AGE_GROUP_LABEL,$TREATMENT_LABEL,
           $ELIG_LABEL;
    if ( ! is_null( $protocol->protocol_no ) ) {
        $protocol_summary = '';
        if ( ! is_null( $protocol->protocol_no ) ) {
            $protocol_summary .= '<p><strong>'.$PROTOCOL_LABEL.'</strong>' . $protocol->protocol_no . '</p>';
        }
        if ( ! is_null( $protocol->secondary_protocol_no ) ) {
            $protocol_summary .= '<p><strong>'.$SPONSOR_LABEL.'</strong>' . $protocol->secondary_protocol_no . '</p>';
        }
        $title = is_null( $protocol->protocol_title ) ? $protocol->protocol_title : $clinical_trial->title;
        if ( ! empty( $title ) ) {
            $protocol_summary .= '<p><strong>'.$PROTOCOL_TITLE_LABEL.'</strong>' . $title . '</p>';
        }
        if ( ! is_null( $protocol->protocol_pi ) ) {
            $protocol_summary .= '<p><strong>'.$PI_LABEL.'</strong>' . $protocol->protocol_pi . '</p>';
        }
        $objective = ! empty( $protocol->protocol_objective ) ? $protocol->protocol_objective : $clinical_trial->objective;
        if ( ! empty( $objective ) ) {
            $protocol_summary .= '<p><strong>'.$OBJECTIVE_LABEL.'</strong>' . $objective . '</p>';
        }
        $description = ! empty( $protocol->lay_description ) ? $protocol->lay_description : $clinical_trial->detailed_description;
        if ( ! empty( $description ) ) {
            $protocol_summary .= '<p><strong>'.$DESCRIPTION_LABEL.'</strong>' . $description . '</p>';
        }
        $phase = ! empty( $protocol->protocol_phase ) ? $protocol->protocol_phase : $clinical_trial->phase;
        if ( ! empty( $phase ) ) {
            $protocol_summary .= '<p><strong>'.$PHASE_LABEL.'</strong>' . $phase . '</p>';
        }
        if ( ! is_null( $protocol->protocol_age ) ) {
            $protocol_summary .= '<p><strong>'.$AGE_GROUP_LABEL.'</strong>' . $protocol->protocol_age . '</p>';
        }
        if ( ! is_null( $clinical_trial->maximum_age ) && ! is_null( $clinical_trial->minimum_age ) ) {
            $protocol_summary .= '<p><strong>'.$AGE_LABEL.'</strong>' . $clinical_trial->minimum_age . ' - ' . $clinical_trial->maximum_age . '</p>';
        }
        if ( ! is_null( $clinical_trial->gender ) ) {
            $protocol_summary .= '<p><strong>'.$GENDER_LABEL.'</strong>' . $clinical_trial->gender . '</p>';
        }
        if ( ! is_null( $protocol->protocol_scope ) ) {
            $protocol_summary .= '<p><strong>'.$SCOPE_LABEL.'</strong>' . $protocol->protocol_scope . '</p>';
        }
        $treatment = ! empty( $protocol->treatment ) ? $protocol->treatment : $clinical_trial->treatment;
        if ( ! empty( $treatment ) ) {
            $protocol_summary .= '<p><strong>'.$TREATMENT_LABEL.'</strong>' . $treatment . '</p>';
        }
        ...
        if ( ! empty( $detailed_elig ) ) {
            ...
            $protocol_summary .= '<p><strong>'.$ELIG_LABEL.'</strong>' . $detailed_elig . '</p>';
        }
        if ( ! is_null( $protocol->disease_site ) ) {
            $protocol_summary .= '<p><strong>'.$CONDITIONS_LABEL.'</strong>' . $protocol->disease_site . '</p>';
        }
        if ( ! is_null( $protocol->protocol_institution ) ) {
            $protocol_summary .= '<p><strong>'.$INST_LABEL.'</strong>' . $protocol->protocol_institution . '</p>';
        }
        if ( ! is_null( $protocol->contact ) ) {
            $protocol_summary .= '<p>' . $protocol->contact . '</p>';
        }
        if ( ! is_null( $protocol->nct_url ) ) {
            $protocol_summary .= '<p><strong>'.$MORE_INFO_LABEL.'</strong>'.$CTGOV_LABEL.' 
...
function build_protocol_obj( $protocol_id, $protocol_no ) {
    global $PROTOCOL_SUMMARY_URL;
    $protocol_obj = new Protocol();
    $url          = esc_url_raw( $PROTOCOL_SUMMARY_URL.$protocol_id.'&protocol_no='.$protocol_no);
    $out          = wp_remote_get( $url );
    if ( ! is_wp_error( $out ) ) {
    ...
    if ( ! is_wp_error( $out ) ) {
        ...
        if ( ! is_null( $xml_doc->getElementsByTagName( 'detailed_description' ) ) ) {
            if ( ! is_null( $xml_doc->getElementsByTagName( 'detailed_description' )->item( 0 ) ) ) {
                $clinical_trial_obj->detailed_description = esc_html($xml_doc->getElementsByTagName( 'detailed_description' )
