				...
				if ( is_object( $row_childs ) ) {
					foreach ( $row_childs as $row_child ) {
						...
						if ( $row_child->getAttribute( 'id' ) === 'protocolList' ) {
							...
							$protocol_list_str .= '<dd>' . $protocol_title . '</dd>';
						}
						$protocol_list_str .= '</dl>';
					}
				}
...
function build_protocol_summary( $protocol, $clinical_trial ) {
	if ( ! is_null( $protocol->protocol_no ) ) {
		...
		$description = ! empty( $protocol->lay_description ) ? $protocol->lay_description : $clinical_trial->detailedDescription;
	...
	if ( ! is_wp_error( $out ) ) {
		...
		if ( ! is_null( $xml_doc->getElementsByTagName( 'eligibility' )->item( 0 ) ) ) {
			$clinical_trial_obj->detailed_eligibility = preg_replace(
				'/[\r\n\r\n]/', '</br>',
				$xml_doc->getElementsByTagName( 'eligibility' )->item( 0 )->getElementsByTagName( 'textblock' )->item( 0 )->textContent
			);
		}
